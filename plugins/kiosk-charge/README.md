...
(`src/lib.rs`), matching the layout of ZeroClaw's own `redact-text` plugin. Shared
primitives come from `crates/kiosk-core` (hand-rolled base58 with golden vectors,
shortvec, Solana Pay URL builder, token-budgeted output shaping — no `solana-sdk`,
wasm32-wasip2-friendly).
...
`kiosk-watch` (T0 payment confirmation the actuation SOP gates on) and `kiosk-attest`
(T1 hash-chained sensor/receipt attestations with a durable nonce) ship in this same
repo. Start at the [top-level README](../../README.md).
